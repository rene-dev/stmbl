...
To use the STMBL you will need to <<Flashing Firmware, flash the firmware>>,
configure the drive for your motor using <<Servoterm>> after determining
the <<Motor, motor parameters>> using the correct
<<HAL (Hardware Abstraction Layer), HAL modules>> for your motor and chosen
control mode and connect the drive, motor and controller using
...
STMBL firmware. Go to the
<<Flashing the F4 (low voltage) board with no bootloader>> section.
...
If the F3 board has only a bootloader flashed and no firmware then the
red LED will flash rapidly  and report as
"STM32 DownLoad Firmware Update:" (Mac)
"0483:df11 STMicroelectronics STM Device in DFU Mode" (Linux lsusb) and
"Unknown Device" in the Windows Device Manager. Use the instructions in
<<Updating Firmware>> in this case. 
...
version) and the STMBL firmware (F3 version). You will need to follow
the instructions in
<<Flashing the F3 (high voltage board) with no bootloader>>
...
Connect 24V to both the F3 and F4 boards and connect with Servoterm
put the F4 board in to Boot Loader mode with the "bootloader" command. 
Servoterm will disconnect. 
...
to program the F4 board and transfer a copy of the F3 software. 
...
=== Flashing the F4 (low voltage) board with no bootloader
...
marked "boot" on the larger of the two boards while the board is powering
up. This is a bit of a fiddle but should only need to be done once when
...
The command to flash the F4 bootloader and firmware is:
...
If required the firmware-only can be flashed with the command

 make btburn
 
But there is no real advantage to this. 
...
The command to flash the bootloader and firmware for the F3 board is:
...
If you type "hv0" at the servoterm command line and see a non-zero voltage
...
* conf0.psi torque constant (Nm/A)
...
* conf0.max_dc_cur max DC current (A)
* conf0.mot_type 0 = ac async, 1 = DC

##### conf0.sin_offset conf0.cos_offset conf0.sin_gain conf0.cos_gain#####
Offsets and gains for analog input. Only needed for sin/cos or if high precision with resolvers is required.
...
In all modes:
* conf0.cur_p pid current proportional gain
* conf0.cur_i pid current integral gain
* conf0.cur_ff pid current ff0 gain

When driven in velocity mode (default)
* conf0.pos_p pid velocity proportional gain (1/s)
* conf0.acc_p pid acceleration proportional gain (1/s)
* conf0.acc_pi pid acceleration integral gain (1/s)

When driven in position mode (e.g. via serial link to LinuxCNC)
* conf0.pos_p pid position proportional gain (1/s)
* conf0.vel_p pid velocity proportional gain (1/s)
* conf0.vel_i pid velocity integral gain (1/s)
* FIXME: conf0.vel_g pid velocity
* FIXME: conf0.g pid
...
* conf0.high_dc_volt brake resistor limit (V)
  * activate brake resistor
* conf0.low_dc_volt voltage limit (V)
  * disable drive
* conf0.max_temp temperature limit (°C)
  * disable drive
* conf0.high_temp temperature limit (°C)
  * reduce max current
* conf0.fan_temp temperature limit (°C)
  * activate fan
...
== Autophasing
Autophasing is the routine used to determine the relative position of encoder
with regard to magnetic orientation of the rotor.
Make sure to set the motor parameters: conf0.r, conf0.l, conf0.j, conf0.polecount before.
Also check feedback before proceeding.
TODO
...
# Version 4 #
The same applys for version 4. additionally, version 4 can also plug directly into a 7I74 or 7I44.

# STMBL config
    # At the beginning of your STMBL config, load sserial:
    link sserial
...
2. Errors
 * Red LED under fan blinks fast - Bootloader but no firmware
