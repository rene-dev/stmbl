...
void mpWindow::OnMagnify( wxMouseEvent &event )
{
   ...
   wxPoint clickPt( event.GetX(),event.GetY() );
   float zoomFactor = event.GetMagnification() * 2.0 + 1.0;
   Zoom(clickPt, zoomFactor);
...
void mpWindow::OnMouseLeftRelease (wxMouseEvent &event)
{
   ...
   {
      Zoom(centerPoint, zoomIncrementalFactor);
   }

   void mpWindow::ZoomOut(const wxPoint& centerPoint )
   {
      Zoom(centerPoint, 1.0 / zoomIncrementalFactor);
   }

   void mpWindow::Zoom(const wxPoint& centerPoint, float zoomFactor)
   {
      ...
      #ifdef MATHPLOT_DO_LOGGING
      wxLogMessage(_("mpWindow::Zoom() prior coords: (%f,%f), new coords: (%f,%f) SHOULD BE EQUAL!!"), prior_layer_x,prior_layer_y, p2x(c.x),p2y(c.y));
      #endif
