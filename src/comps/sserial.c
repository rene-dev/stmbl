...
uint8_t sserial_slave[] = {
    0x06, 0x09, 0x43, 0x01, 0x59, 0x01, 0x00, 0x00,
    ...
    0x12, 0x01, 0x00, 0x70, 0x61, 0x64, 0x64, 0x69,
    0x6E, 0x67, 0x00, 0x00, 0xA0, 0x02, 0x00, 0x80,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x2B, 0x01, 0x00, 0x70, 0x61, 0x64, 0x64, 0x69,
    0x6E, 0x67, 0x00, 0x0C, 0x00, 0x2C, 0x00, 0x48,
    0x00, 0x68, 0x00, 0x88, 0x00, 0xA4, 0x00, 0xC4,
    0x00, 0xE0, 0x00, 0x14, 0x01, 0x2C, 0x01, 0x00,
    0x00, 0x00, 0x01, 0x00, 0x00,
};
uint16_t sserial_ptocp = 0x0143;
uint16_t sserial_gtocp = 0x0159;
...
typedef struct {
  ...
  uint32_t index_enable : 1;
  uint32_t padding : 2;
} sserial_in_process_data_t;  //size:5 bytes
_Static_assert(sizeof(sserial_in_process_data_t) == 5, "sserial_in_process_data_t size error!");
//******************************************************************************
    ...
    if(available >= 1) {
      ...
      if(lbp.ct == CT_LOCAL && lbp.wr == 0) {  //local read, cmd+crc = 2b
        ...
        if(lbp.byte == UnitNumberRPC && available >= 2) {  //unit number, cmd+crc = 2b
          ...
          // if(PIN(index_clear) > 0) {
          //   data_in.index_enable = 0;
          // } else {
          //   data_in.index_enable = data_out.index_enable;
          // }
